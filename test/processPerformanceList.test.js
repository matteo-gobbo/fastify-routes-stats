...
test('processPerformanceList', async t => {
  ...
  await t.test('standard deviation', t => {
    t.plan(1)
    t.assert.equal(processPerformanceList([-2, -1, 0, 1, 2]).sd, Math.sqrt(2.5))
  })

  await t.test('max', t => {
    t.plan(2)
    t.assert.equal(processPerformanceList([-2, -1, 0, 1, 2]).max, 2)
    t.assert.equal(processPerformanceList([6, 10, 2, 5]).max, 10)
  })

  await t.test('min', t => {
    t.plan(2)
    t.assert.equal(processPerformanceList([-2, -1, 0, 1, 2]).min, -2)
    t.assert.equal(processPerformanceList([6, 10, 2, 5]).min, 2)
  })
})
